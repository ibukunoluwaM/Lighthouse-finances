...
export default function ActiveAsssetUsersChart() {
  const monthNames = [
    "Jan",
    "Feb",
    "Mar",
  ...
  let endValue = 3271; // fixed December target
  let months = 12;
  ...
  for (let i = 0; i < months; i++) {
    if (i === 0) {
      ...
      // Force December to be exactly 3271
      value = endValue;
      data.push({ name: monthNames[i], value: [monthNames[i], value] });
    } else {
    ...
    series: [
      {
        ...
        type: "line",
        data: data.map((item) => item.value), // ["Jan", 200], ..., ["Dec", 3271]
        showSymbol: true,
        smooth: true,
        lineStyle: {
          color: "#a1354f",
